...
{
    //char buf[] ="/home/deeplearning/DeepLearning/sort/data/img1/000015.jpg";
    int i = 0;
    char *p = strtok (buf, "/.");
    char *array[20];
    ...
    int last_but_one_index = i - 2;

    //printf("framenumber is");
    //printf("%s\n", array[last_but_one_index]);

    return array[last_but_one_index];
}*/



int read_file_framesize (const char* file_name)
{
    char count_command[100] = "ls ";
    strcat(count_command, file_name);
    
    char wc[50] = " | wc -l > frame_count.txt";
    strcat(count_command, wc);

    system(count_command);

    FILE* count_txt = fopen("frame_count.txt", "r");
    int i=0;
    fscanf(count_txt, "%d", &i);
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    //while(1){
      //  if(filename){
        //    strncpy(input, filename, 256);
            //framenumber = getFilename(filename);
            //printf("%ld",framenumber);
        //} else {
    ...
    //char startframe = "000339";
    //char endframe = "002129";

    //int stframe = startframe - '0';
    //int enframe = endframe - '0';



    int stframe = 0;
    int enframe = read_file_framesize(filename);

    //printf ("Framesize\n");
    //printf ("%d", enframe);
    //printf ("Framesize is done\n");

    int count;
    for(count = stframe; count < enframe; count++){

        char frame[10];
        char path[80];
        strcpy(path, filename);
        sprintf(frame, "%d", count);
        //strcat(path, "/");
        strcat(path, frame);
        strcat(path, ".jpg");
	...
	image sized = letterbox_image(im, net.w, net.h);
        ...
        //framenumber = getFilename(input);
        draw_detections_detector(im, l.w*l.h*l.n, thresh, boxes, probs, names, alphabet, l.classes, count, filename);
        /*if(outfile){
