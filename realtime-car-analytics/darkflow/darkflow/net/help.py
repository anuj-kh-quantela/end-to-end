...
def camera(self):
    ...
    if file == 'camera':
        ...
        saveAs = os.path.basename(file)
        saveAs = saveAs[:-4]
        if not os.path.exists("data/" + saveAs + "/output"):
            os.makedirs("data/" + saveAs + "/output")
    ...
    if self.FLAGS.csv :
        f = open("data/" + saveAs + "/output" + "/output_{}.csv".format(saveAs),"w")
    ...
    if SaveVideo:
        ...
        videoWriter = cv2.VideoWriter(
            "data/" + saveAs + "/output" + "/output_{}.avi".format(saveAs), fourcc, fps, (width, height))
